use {
    super::{Lexer, Node, Parse},
    crate::{Diagnostic, FileId, Span},
...
impl<'source> Parser<'source> {
    ...
    pub(super) fn parse_root(&mut self) {
        self.builder.start_node(Node::Root.into());
    ...
    fn parse_indentation(&mut self) {
        let indent = if self.peek == Some(Node::Whitespace) {
        ...
        if indent > self.indent {
            ...
            while indent > self.indent {
                self.builder.start_node(Node::Block.into());
                self.indent += 1;
            }
        }
        if self.peek == Some(Node::Whitespace) {
            self.bump();
        }
        if indent < self.indent {
            while indent < self.indent {
    ...
    fn parse_def(&mut self, in_group: bool) {
        ...
        if !is_proc {
            self.builder.start_node_at(checkpoint, Node::Call.into());
            self.bump_arguments(in_group);
            self.builder.finish_node();
            return;
        }

        self.builder.start_node_at(checkpoint, Node::Def.into());

        self.builder.start_node_at(checkpoint, Node::Proc.into());
        self.builder.finish_node();
        assert_eq!(self.peek, Some(Node::Colon));
        self.bump();
    ...
    fn try_bump_params(&mut self, in_group: bool) -> bool {
        loop {
            match self.peek {
                Some(Node::Identifier | Node::Whitespace) => self.bump(),
                Some(Node::Colon) => return true,
                Some(Node::String | Node::Number | Node::ParenOpen) => return false,
                Some(Node::Newline) | None if !in_group => {
                    return false;
                }
                Some(Node::ParenClose) if in_group => {
                    return false;
                }
                Some(Node::Newline) | None => {
                    self.error("unexpected newline");
                    return false;
                }
                Some(Node::ParenClose) => {
    ...
    fn bump_arguments(&mut self, in_group: bool) -> usize {
        ...
        loop {
            match self.peek {
                ...
                Some(Node::Identifier | Node::String | Node::Number | Node::Whitespace) => {
                    count += 1;
                    self.bump();
                }
                Some(Node::Newline) | None if !in_group => {
    ...
    fn parse_group(&mut self) {
        self.builder.start_node(Node::Group.into());
        assert_eq!(self.peek, Some(Node::ParenOpen));
        self.bump();
