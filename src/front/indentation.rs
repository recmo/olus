...
pub struct Lexer<'source> {
    logos: LogosLexer<'source, Kind>,

    /// Buffer of tokens that have been read ahead
    buffer: Vec<(Kind, Span)>,
...
impl Iterator for Lexer<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let token = self
            .logos
            .next()?
            .unwrap_or_else(|err| err.unwrap_or(Kind::ErrorUnknownToken));
        ...
        if indentation.len() > last.len() {
            if indentation.starts_with(last) {
                ...
                self.buffer.push((Kind::Indent, indentation_span));
            } else {
                // Indentation does not have common prefix with last indentation.
                self.buffer
                    .push((Kind::ErrorInconsistentIndentation, indentation_span));
            }
            ...
            .rposition(|i| i == indentation)
        {
            ...
            for _ in 0..dedents {
                self.buffer.push((Kind::Dedent, indentation_span));
            }
        } else {
        ...
        Some((Kind::Newline, newline_span))
    }
}
